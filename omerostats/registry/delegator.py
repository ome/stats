...
from omerostats.registry.models import Hit, IP
...
def month_year_table(startdate, enddate, agents):
    ...
    for year, month in month_year_iter(startdate.month, startdate.year, enddate.month, enddate.year):
...
def year_table(startdate, enddate, agents):
    ...
    for year in month_year_iter(startdate.year, enddate.year):
        idx = "%s" % (year)
        table[idx] = dict()
        table[idx]['All*'] = {'total':0, 'unique':0}
        for a in agents:
