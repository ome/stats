...
SCHEMA_DIR = os.path.join(os.path.dirname(__file__), 'schema').replace('\\','/')
...
logger.debug("Loading Image for Tiff Support")
...
logger.debug("Loading lxml for XML Schema Validation Support")
haveXsdSupport = True
try:
...
class ParseMessage(object):
	...
	def __init__(self, inFilename,inLine,inColumn, inErrorType, inContext, inMessage):
		...
		'''
		logger.debug("inFilename: '%s',  inLine:'%s', inColumn:'%s', inErrorType:'%s', inContext:'%s', inMessage:'%s'" % (inFilename,inLine,inColumn, inErrorType, inContext, inMessage))
