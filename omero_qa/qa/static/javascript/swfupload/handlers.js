...
function queueComplete(numFilesUploaded) {
	...
	numFilesUploaded = numFilesUploaded + Math.round(status.innerHTML.match(/[0-9]+/));
	var token = status.innerHTML.match(/\w{32}/)
	if (numFilesUploaded > 0) {
	    status.innerHTML = numFilesUploaded + " file" + (numFilesUploaded === 1 ? "" : "s") + " uploaded.";// (token:" + token + ")";
	} else {
